...
describe('Sida2', () => {
	...
	it('visar felmeddelande om API-anropet misslyckas', async () => {
		...
		vi.spyOn(api, 'getExampleData').mockRejectedValueOnce(new Error('Misslyckades att hämta data'));
		...
		await waitFor(() => {
			expect(screen.getByText(/Misslyckades att hämta data/i)).toBeInTheDocument();
		});
	});
});
